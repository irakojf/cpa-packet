"""General ledger CLI command wrapper."""
...
def register_general_ledger_command(cli_group: click.Group) -> None:
    """Add `cpapacket general-ledger` command to the provided click group."""

    @cli_group.command("general-ledger")
    ...
    def general_ledger_command(
        ...
        """Generate General Ledger deliverable outputs."""
        ...
        result = GeneralLedgerDeliverable().generate(run_context, providers, prompts={})

        if not result.success:
            raise click.ClickException(result.error or "General ledger generation failed.")

        click.echo("General ledger deliverable complete.")
